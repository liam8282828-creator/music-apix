...
function showPage(page) {
  ...
  document.getElementById("page-" + (page === "app" ? "dashboard" : page)).style.display =
    (page === "login" || page === "register") ? "flex" : "flex"
}
...
async function loadStats() {
  ...
  const since = new Date(currentUser.createdAt || currentUser.created_at).toLocaleDateString("es-ES")
...
async function doRegen() {
  closeModal()
  try {
    const data = await fetch(API + "/api/keys/regenerate", { method: "POST", headers: authHeaders() }).then(r => r.json())
    apiKeyValue = data.key
    document.getElementById("api-key-input").value = data.key
    document.getElementById("api-key-input").type = "password"
    keyVisible = false
...
async function searchMusic() {
  ...
  try {
    const results = await fetch(API + "/api/music/search?q=" + encodeURIComponent(q), { headers: authHeaders() }).then(r => r.json())
...
function loadSettings() {
  ...
  document.getElementById("set-since").textContent = new Date(
